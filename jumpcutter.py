...
def main():
    ...
    parser.add_argument('--split_chunks', type=int, default=1, help="number of chunks to split to")
    ...
    command = "python ../ffmpeg-split.py -f " + INPUT_FILE + " -c " + str(CHUNKS)
    subprocess.call(command, shell=True)

    INPUT_FILE_ORIGINAL = INPUT_FILE
    for iteration in range(1, CHUNKS + 1):
        INPUT_FILE = INPUT_FILE_ORIGINAL
        ...
        if CHUNKS != 1:
            ...
            INPUT_FILE = INPUT_FILE[:dotIndex] + "-" + str(iteration) + "-of-" + str(CHUNKS) + INPUT_FILE[dotIndex:]
            dotIndex = OUTPUT_FILE.rfind(".")
            OUTPUT_FILE = OUTPUT_FILE[:dotIndex] + str(iteration) + OUTPUT_FILE[dotIndex:]

        TEMP_FOLDER = "TEMP_" + INPUT_FILE
